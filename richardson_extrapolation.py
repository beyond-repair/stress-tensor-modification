...
def extrapolate_sequence(
    ...
    Parameters
    ----------
    values : sequence of observed scalar quantities ordered from
             coarsest to finest mesh.
    refinement_ratio : nominal r (default 2). For depth-driven meshes
             this is an effective ratio; the observed order is still
             computed from the data.
    assumed_order : if given, use this p for the final extrapolation
             instead of the observed order (useful when theory predicts
             a known rate, e.g. p=1 for piecewise-constant collocation).

    Returns
    -------
    dict with keys:
      values          : input list
      observed_orders : list of p estimates from successive triplets
      p_used          : order used for final extrapolation
      Q_star          : extrapolated continuum estimate (or NaN)
      relative_changes: successive |Q_{i+1}-Q_i| / |Q_i|
...
def _self_test() -> None:
    """Analytic check: Q(h) = 1 + 2*h + 3*h**2, r=2, exact p=1 for leading term."""
    print("richardson_extrapolation self-test …")
    # Leading linear term dominates for moderate h
    hs = [1.0, 0.5, 0.25]
